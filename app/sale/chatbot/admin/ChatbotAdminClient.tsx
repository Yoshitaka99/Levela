...
const adminStarters = [
  {
    icon: Database,
    text: "顧客シートをつなぐ前提で、営業用ボットの流れを作って",
  },
  {
    icon: CalendarClock,
    text: "予約や日程調整もできるボットにするなら何APIが必要？",
  },
  {
    icon: Braces,
    text: "まずローカルで動く最小構成を説明して",
  },
];
  ...
  return (
    <main className="dark min-h-screen bg-slate-950 text-slate-50">
      <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col px-4 py-5">
        <header className="flex flex-col gap-3 border-b border-border pb-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
              API接続、運用カテゴリ、実データ連携の確認用画面です。一般ユーザーには見せない前提の操縦席です。
            </p>
          </div>
          <Button
            asChild
        ...
        <section className="grid min-h-0 flex-1 gap-4 py-4 lg:grid-cols-[280px_1fr]">
          <aside className="rounded-lg border border-border bg-card p-3">
            ...
            <div className="space-y-2 text-sm">
              ...
              <ApiSlot name="Google Calendar" state="mock tool" />
            ...
            <div className="mt-4 rounded-md border border-border bg-background p-3 text-xs leading-5 text-muted-foreground">
              実APIを使う場合は <code>CHATBOT_LIVE_OPENAI=true</code> を有効にします。
            </div>
          </aside>
              ...
              <ConversationContent className="min-h-full gap-5 p-4 md:p-6">
                {messages.length === 0 ? (
                  <ConversationEmptyState
                    ...
                    title="管理用プロンプトを試す"
                    description="接続したいAPIや業務フローを入力してください。"
                  >
            ...
            <div className="border-t border-border p-3 md:p-4">
              ...
              >
                <PromptInputTextarea
                  ...
                  placeholder="例：営業相談ボットとして、顧客シートとカレンダーをつないだ形にしたい"
