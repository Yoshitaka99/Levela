...
function organizeLocally(memo: string) {
  ...
  for (const line of lines) {
    ...
    if (/(貯金|預金|資産|NISA|ニーサ|iDeCo|積立|投資|黒字|余剰)/i.test(line)) {
  ...
  if (!form.income.trim()) {
    appendLine(form, "income", "本人年収：年齢別平均給与で補完（収入未入力）");
  }
...
async function organizeWithOpenAI(memo: string) {
  ...
  const response = await client.responses.create({
    ...
    input: [
      ...
      "- 収入が未入力の場合は、incomeに「本人年収：年齢別平均給与で補完（収入未入力）」を入れる。固定の478万円は入れない",
      "- 年表の収入は国税庁の年齢階層別平均給与カーブに沿わせる前提で整理する",
      "- 月々の貯金額、毎月の貯金、毎月の積立、NISA月額がある場合は、assetsに必ず残す",
      "- 月々の貯金額がある場合、現在支出は「現在年収 - 月々の貯金額×12」で逆算する前提にする",
