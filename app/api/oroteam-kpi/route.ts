...
export async function POST(request: Request) {
  try {
    const body = (await request.json().catch(() => ({}))) as Partial<OroGoalConfig> & { seatRate?: string | number | null };
    const month = typeof body.month === "string" && /^\d{4}-\d{2}$/.test(body.month) ? body.month : DEFAULT_MONTH;
    const seatRate = typeof body.seatRate === "string" || typeof body.seatRate === "number" ? String(body.seatRate) : null;
    ...
    const data = await getOroTeamKpiData({ month, seatRate });
