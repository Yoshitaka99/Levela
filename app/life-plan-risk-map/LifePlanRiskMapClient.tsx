...
const sections: Section[] = [
  ...
  },
  {
    ...
    number: "8",
    title: "⭐",
    guide: "経験、好きなこと、感情、願望、不安、やれていない事、やりたい事",
    placeholder:
...
const simpleMemoHints: SimpleMemoHint[] = [
  ...
  {
    title: "あなたの強み",
    items: ["経験", "感情", "願望", "現状の不安", "やれていない事", "やりたい事"],
  },
];
...
const simpleMemoPlaceholder = [
  ...
  "文字起こしメモ：今のままだと不安。SNS発信はまだできていないけど、家族との時間を増やしたい。昔から人の相談に乗ることが多い。",
].join("\n");
...
function buildIncomeCalculationRules() {
  return [
    "【収入計算ルール】",
    "・収入未入力の場合は、国税庁の平均給与478万円を初期値にする。",
    "・年表の収入は国の平均給与上昇率3.9%/年に合わせて自然に増やし、不自然な急増はさせない。",
...
【計算前提】
・差分は「収入 - 支出」
・収入未入力の場合は、日本の平均年収478万円を入れる
・年表の収入は平均給与上昇率3.9%/年に合わせて自然に増やす
・住宅を後々購入する場合、購入額をその年から35年ローンで割った金額を支出に入れる
...
・B：同じ悩みの構造を持つ人に刺さる
・C：同じ人生テーマの人に刺さる
・各A/B/C人物カードに「こんな感情を受けた人：〇〇」を短く表示する
...
文字起こしから、感情・願望・不安・未実行・やりたい事を拾い、短いタグに圧縮する。
