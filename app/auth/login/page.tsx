  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      setTimeout(() => {
        ...
        router.refresh(); // ヘッダー等のログイン状態を更新するため
      }, 1000);
    }
    setLoading(false);
  };
    ...
    <div className="w-full max-w-md p-8 space-y-8 bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl animate-in fade-in zoom-in duration-500">
      <div className="text-center">
        {redirectParams ? (
          <>
            ...
            <p className="text-sm text-gray-400">出品者に連絡するにはログインしてください</p>
          </>
        ) : (
          <>
      ...
      <form onSubmit={handleLogin} className="space-y-6 mt-8">
        <div className="space-y-4">
          ...
          <div>
            <label className="text-sm font-medium text-gray-300 block mb-2">パスワード</label>
            <input
              type="password"
              placeholder="••••••••"
              value={password}
      ...
      <p className="mt-8 text-center text-sm text-gray-400">
        アカウントをお持ちでないですか？{" "}
        <Link href={`/auth/signup${redirectParams ? `?redirect=${redirectParams}` : ''}`} className="font-medium text-orange-400 hover:text-orange-300 transition-colors">
          新しく登録する
        </Link>
      </p>
    </div>
