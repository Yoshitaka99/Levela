...
## /report-guard (比較検証版) について

`/report-guard` は同じデータソース・同じWeb Appを使う受信箱型UIの別実装。
トリアージ状態 (問題なし/要調査/虚偽確定) は `updateReply` を利用して
返信チェックタブに `TRIAGE|行キー|` のキーで保存する。

アイディアメモ (未実装): 差分の自動検知をDiscordへ通知する。既存のDiscord bot
構成に相乗りでき、ページを開かなくても検知が届くようになる。
